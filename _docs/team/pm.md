...
- Keep the group label, and make sure the issue carries `MVP` or
  `post-MVP`
...
- Everything moved out of scope links to a follow-up issue, labeled
  `post-MVP`
- The issue itself carries `MVP` or `post-MVP`
...
File a follow-up issue, label it `post-MVP`, and list it under out of
scope with a link to that issue, so it is clear what was moved and
where it went.